...
import 'dart:io';
import 'package:adams/Service/Session/Impl/SessionImpl.dart';
import 'package:adams/Service/Session/Session.dart';
...
//                                                                              new NetworkImage(userImageUrl),
                                                                          new NetworkImage("https://res.cloudinary.com/sewwandi/image/upload/v1610136475/Myimage.jpg"),
                                                                        ))),
                                                              ),
                                                            ),
                                                          ]),
...
// ************------------------------ TTS end
  ...
  Future<void> send() async {
    ...
    if (userQuestion.text != null) {
      a = userQuestion.text;
//      _session = await  session.mainRunnerSession(a, _session, userEmail );
      s = await session.mainRunnerSession(a, _session);
      b = await session.mainRunner(a, s);
      v = true;
      _speak();
    }
    setState(() {
